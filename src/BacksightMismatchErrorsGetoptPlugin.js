  ...
  apply(program) {
    program.plugin('configureGetopt', function (getopt) {
      getopt.options.push(
        [
          '',
          'warn-bs-diff=DEGREES',
          `warn if backsights disagree by more than DEGREES (default: ${DEFAULT_WARN_DIFF}°)`,
        ],
        [
          '',
          'error-bs-diff=DEGREES',
          `error if backsights disagree by more than DEGREES (default: ${DEFAULT_ERROR_DIFF}°)`,
        ]
      )
    })
    ...
    program.plugin('gotopt', function (opt) {
      program.apply(
        new BacksightMismatchErrorsPlugin({
