...
export default class BreakoutOutputGetoptPlugin {
  apply(program: Tapable) {
    program.plugin(
      'configureGetopt',
      function (getopt: { options: Array<Array<string>> }) {
        getopt.options.push(
          [
            '',
            'filename-only-survey-scans',
            'use filenames of survey scans only',
          ],
    ...
    program.plugin(
      'gotopt',
      function (opt: { options: { [name: string]: any } }) {
        program.apply(
          new BreakoutOutputPlugin({
