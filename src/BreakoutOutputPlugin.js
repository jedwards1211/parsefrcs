...
export default class BreakoutOutputPlugin {
  ...
  convertTripHeader(trip: TripWithSummary): Trip {
    ...
    if (surveyScan && this.options.basenameOnlySurveyScans)
  ...
  apply(program: Tapable) {
    ...
    program.plugin('parser', (parser: Tapable) => {
      ...
      parser.plugin(
        'calculatedShot',
        (shot: CalculatedShot): CalculatedShot => {
          if (zeroReference) {
      ...
      //   return _comment
      // })
      parser.plugin('shot', (shot: Object): Object => {
        ...
        if (
          ...
          (!lastFromStation.cave ||
            lastFromStation.cave === this.currentCaveName) &&
          ...
          if (
            lastToStation &&
            (!lastToStation.cave ||
              lastToStation.cave === this.currentCaveName) &&
          ...
          if (shot.to) {
            ...
            measurements = (fromStation.splays ||
              (fromStation.splays = []): any)
          }
        }
        ...
        if (measurements) {
          ...
          if (Number.isFinite(shot.azmFs))
            frontsight.azm = shot.azmFs.toFixed(2)
          ...
          if (Number.isFinite(shot.azmBs) || Number.isFinite(shot.incBs)) {
            ...
            if (Number.isFinite(shot.azmBs))
              backsight.azm = shot.azmBs.toFixed(2)
